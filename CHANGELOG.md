...
### 🧼 Generic starter-pack release

- **Code genericization** — deployment-specific defaults replaced with generic
  ones (LLM proxy URL, knowledge-broker, Qdrant UI, voice bridge, NFS root,
  media paths); frontend report links are now relative; OpenAPI servers are
  relative; deployment endpoints move to compose/env where they belong
- **New guide:** `docs/going-further.md` — what the starter pack includes and
  how to wire your own tools, personas, LLM proxy, QA runtime, and heartbeat
  orchestration
- **Slimmer public tree** — internal deploy wrapper, dead code, and
  environment-specific test fixtures removed from the release

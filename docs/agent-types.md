...
Agent Types are first-class entities in ClawBoard that define AI persona templates for agent tasks. They're sourced from the [nim/agency-agents](https://git.skyday.eu/nim/agency-agents) repository (forked from msitarzewski/agency-agents) and synced into the database on startup.
...
curl -s https://nimspace.skyday.eu/dashboard-dev/api/agent-types | python3 -m json.tool
```
...
```bash
# Via API
curl -X POST https://nimspace.skyday.eu/dashboard-dev/api/agent-types/sync

# Or pull latest from Gitea and restart backend
...
1. Clone `ssh://git@git.skyday.eu:222/nim/agency-agents.git`

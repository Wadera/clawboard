...
DASHBOARD_PASSWORD_HASH=changeme         # bcrypt hash: node scripts/hash-password.js yourpassword
                                         # (or run ./setup.sh, which generates it for you)
```
...
2. **Gateway WebSocket** — Live status updates from the OpenClaw gateway (default: `ws://localhost:18789`)

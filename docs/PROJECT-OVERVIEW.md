...
| `PATCH /api/tasks/:id` | Updates incl. status transitions with notes |
...
4. **Agents work independently** — spawn, work, then hand off in `review`
5. **Orchestrator verifies and completes** — reviews subtasks, runs checks, approves or rejects, then completes the task
...
task management CLI complete-subtask abc123 0   # Mark subtask ready for review
...
Fresh installs: `./setup.sh` then `docker compose up -d` — see [DEPLOYMENT.md](../DEPLOYMENT.md).
Managed production deployments should go through a release wrapper rather than hand-run compose commands.

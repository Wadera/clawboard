...
summary through the LLM-free `hermes send` CLI as the `hermes` user
(`sudo -n -u hermes env HOME=/home/hermes /home/hermes/hermes-agent/venv/bin/hermes send --to TARGET`).
The target defaults to `discord:1465806566350651484` (hermes target format is
`platform:chat_id`) and can be overridden with

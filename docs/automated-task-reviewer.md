...
When review requires authenticated UI validation or secret retrieval, the QA reviewer typically needs deployment-specific helper tooling, e.g.:

- a shared browser instance (opened and reached over CDP) for authenticated UI checks
- a password-vault CLI session for retrieving test credentials

Operational constraint to plan for: if those helpers are only executable by the
reviewer's dedicated OS user, the review pipeline must run in that user's
context (or via a controlled wrapper) — other service accounts will get
`Permission denied`.

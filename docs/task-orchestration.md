...
When browser/auth validation is expected, task instructions should explicitly allow the QA reviewer to use the shared Spark browser and runtime Bitwarden retrieval rather than assuming the implementing agent's own claims are sufficient.
...
For Hermes-based QA on this homelab, authenticated browser evidence should prefer the shared Spark Chromium session plus runtime Bitwarden retrieval when needed. If the reviewer cannot reach the required browser/auth path independently, prefer `reject` or `escalate` over trusting the implementing subagent's own validation claims.

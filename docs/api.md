...
- Public base: `https://nimspace.skyday.eu/api` (nginx strips `/api`; backend routes have no prefix). Dev stack: `/api/dev`.
...
a Webhook-node URL + the signature check covers automation.skyday.eu flows.

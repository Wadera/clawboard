...
/**
 * OrbAvatar — renders the claw-orb iframe once and uses postMessage for state updates.
 * Falls back to the CSS StatusOrb when the claw-orb plugin isn't available.
 */
function OrbAvatar({ state, plugins: pluginsList }: { state: string; plugins: { name: string; healthy: boolean }[] }) {
  const iframeRef = useRef<HTMLIFrameElement>(null);
  const orbPlugin = pluginsList.find(p => p.name === 'claw-orb' && p.healthy);
  ...
  if (orbPlugin) {
    return (
      <iframe
        ref={iframeRef}
        src={`${import.meta.env.VITE_API_BASE_URL || '/api'}/plugins/claw-orb/avatar?state=${initialStateRef.current}`}
        style={{
          width: 200,
          height: 200,
          border: 'none',

...
import { X, Clock, CheckCircle2, Circle, Cpu, Calendar, Link as LinkIcon, GitBranch, FileText, Globe, Box, Plus, ExternalLink, AlertTriangle, Lightbulb, Archive, Play, Pencil, Clipboard, ScrollText, Settings, Trash2, ArchiveRestore, Wrench, Search, Unlink, BookOpen } from 'lucide-react';
...
export const ProjectDetailModal: React.FC<ProjectDetailModalProps> = ({ project, onClose }) => {
  ...
  const [activeTab, setActiveTab] = useState<'overview' | 'resources' | 'tasks' | 'sessions' | 'links' | 'files' | 'context' | 'tools' | 'reports'>('overview');
  ...
  const fetchProjectReports = useCallback(async () => {
    ...
    try {
      ...
      console.error('Failed to fetch project reports:', e);
    } finally {
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="project-detail-modal" ref={modalRef} onClick={(e) => e.stopPropagation()}>
        ...
        <div className="project-detail-tabs">
          ...
          <button className={`tab ${activeTab === 'reports' ? 'active' : ''}`} onClick={() => { setActiveTab('reports'); fetchProjectReports(); }}>
            <BookOpen size={14} /> Reports
            {projectReports.length > 0 && <span className="tab-badge">{projectReports.length}</span>}
          </button>
        </div>
          ...
          {activeTab === 'reports' && (
            <div className="reports-tab">
              <div className="reports-tab-header">
                <h3 className="reports-tab-title">Project Reports</h3>
                <Link
              ...
              {reportsLoading ? (
                ...
                <div className="tab-empty">
                  <BookOpen size={32} className="tab-empty-icon" />
                  <p>No reports linked to this project</p>
                  <p className="tab-empty-hint">Create a report and assign it to "{project.name}"</p>
                </div>
              ) : (

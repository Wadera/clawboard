...
# Generate plugins config if not exists
if [ ! -f clawboard.plugins.json ]; then
    if [ -f clawboard.plugins.example.json ]; then
        cp clawboard.plugins.example.json clawboard.plugins.json
        echo -e "${GREEN}✅ Created clawboard.plugins.json from example (no plugins by default)${NC}"
        echo -e "${BLUE}   Edit clawboard.plugins.json to add external plugins${NC}"
    fi
    echo ""
fi
...
echo "   • Plugins: ./clawboard.plugins.json (no plugins by default)"
echo ""
...
echo "   • Add external plugins via clawboard.plugins.json (see docs/plugin-development.md)"
echo ""

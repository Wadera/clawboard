...
> **⚠️ NimSpace production (nimspace.skyday.eu): do NOT follow the compose
> examples below to deploy.** Production on the AI VM is deployed exclusively
> via `sudo /usr/local/sbin/clawboard-deploy-main <origin-main-sha>` from the
> dedicated exact-main deployment clone — see `docs/repo-layout.md`. The
> examples in this guide are for standing up a fresh ClawBoard installation
> elsewhere.

...
> **⚠️ Managed deployments:** if this checkout is a wrapper-managed production
> deployment, do not `docker compose up` by hand — deploy exclusively via your
> release wrapper. The examples in this guide are for standing up a fresh
> ClawBoard installation.

Complete infrastructure is in place for deploying ClawBoard from scratch.
...
docker compose up -d
```
Access: `http://localhost:8082/dashboard/`
...
FRONTEND_PORT=8082  # Internal port, Traefik handles external access
```
...
# bcrypt hash — generated by setup.sh, or: node scripts/hash-password.js yourpassword
...
1. **Verify Traefik/nginx** — Test SSL certificate generation for your domain
2. **Test backup/restore** — Ensure the database scripts work in your environment
3. **Set up monitoring** — Watch the `/health` endpoints and container health checks

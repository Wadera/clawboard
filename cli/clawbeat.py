...
# under /var/log/clawboard/clawbeat/. The legacy /tmp paths are kept for
...
DURABLE_STATE_DIR = Path("/var/log/clawboard/clawbeat")
...
DASHBOARD_URL = "https://your-dashboard.example.com/dashboard"
# Discord user target for blocked-task notifications (Wadera).
DISCORD_NOTIFY_USER = "user:YOUR_DISCORD_USER_ID"
...
def should_route_orchestration_wake(task: dict) -> tuple[bool, str]:
    ...
    Hermes worker with access to browser + vault rather than by the
...
def launch_hermes_qa_wake(message: str, task_id: str | None = None,
    ...
    Runs as the hermes OS user so the worker can access shared-browser browser helpers,
    password-vault helpers, and Hermes runtime state directly. This is intentionally
    ...
    cmd = [
        ...
        "VAULT_CLI_APPDATA_DIR=~/.config/vault-cli",
...
def build_hermes_wake_footer() -> str:
    ...
    return "\n".join([
        ...
        "/var/log/clawboard/orchestration-actions.log AND write the dedup "
...
def build_review_prompt(task: dict, context: dict,
    ...
    lines.extend([
        ...
        "```bash",
        "~/tools/browser-open 'https://your-dashboard.example.com/dashboard-dev/'",
        "~/tools/cdp-tunnel 9223",
        "~/tools/bw-ensure-session >/dev/null",
        "```",
        "Operator approval is already granted for shared-browser and password-vault use in this smoke task.",
...
def build_access_override_guidance(task: dict) -> list[str]:
    ...
    if execution.get("allow_override_at_spawn"):
        lines.extend([
            ...
            "Prefer the host-side Hermes QA/orchestrator path you are already running in for vault/browser/SSH checks; it has the real homelab access that the backend-container implementation worker may lack.",
            ...
            "```",
            "Only add valid ClawBoard capability tags. Put specific missing tools such as password-vault, SSH, n8n API, and NFS verification into execution notes / reports rather than invalid capability tags.",
        ])
    else:
        lines.extend([
    ...
    lines.extend([
        ...
        "```",
        "Do not use sudo for ClawBoard/browser/vault helper commands in the Hermes QA worker; it is already running as the hermes OS user with the intended helper access.",
    ])
    return lines

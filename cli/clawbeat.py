...
BLOCKED_NOTIFY_FILE = Path("/tmp/clawbeat-blocked-notified.json")

# ClawBoard dashboard base URL — used to build per-task deep links in notifications.
DASHBOARD_URL = os.environ.get("CLAWBOARD_DASHBOARD_URL", "http://localhost:3000/dashboard")
    ...
    try:
        ...
        for session in sessions:
            if session.get("key") == "agent:main:main":
                ...
                log(f"Main session age: {age_ms}ms, "
...
def deliver_to_gateway(message: str, wake_now: bool = True) -> bool:
    """Deliver a message to the main session via OpenClaw cron one-shot job.

    Creates a one-shot cron job that fires immediately and auto-deletes.
    - wake_now=True: immediate wakeup (for idle users, forces response)
    - wake_now=False: next-heartbeat delivery (for active users, background)

    Returns True on success, False on failure.
    """
    if DRY_RUN:
        mode = "now" if wake_now else "next-heartbeat"
        cron_log(f"DRY RUN: Would deliver to gateway (wake={mode}): "
                 f"{message[:200]}...")
        return True
    ...
    cmd = [
        ...
        "--description", "Auto-generated by clawbeat --cron",
    ]
    ...
    try:
        ...
        if result.returncode != 0:
            cron_log(f"Failed to create cron job: {result.stderr[:300]}")
            return False

        cron_log(f"Delivered to gateway (wake={wake_mode}): "
                 f"{message[:100]}...")
        return True

    except subprocess.TimeoutExpired:
        cron_log("Cron job creation timed out")
        return False
    except FileNotFoundError:
        cron_log("openclaw binary not found")
        return False


def cron_deliver(action: str, reason: str, message: str = None,
                 wake_type: str = None, task_id: str = None) -> None:
    """Deliver clawbeat result via gateway in cron mode.

    - HEARTBEAT_OK → log silently and exit
    - WAKE → check user activity and deliver appropriately
    """
    if action == "HEARTBEAT_OK":
        cron_log(f"OK: {reason}")
        sys.exit(0)

    # WAKE action — need to deliver
    if not message:
    ...
    if user_active:
        # Active user → background delivery (systemEvent, next-heartbeat)
        cron_log(f"User ACTIVE — delivering as background event: {reason}")
        success = deliver_to_gateway(message, wake_now=False)
    else:
        # Idle user → immediate delivery (systemEvent, wake now)
        cron_log(f"User IDLE — delivering as immediate wake: {reason}")
        success = deliver_to_gateway(message, wake_now=True)
...
def record_blocked_human_notified(task_id: str, task_title: str) -> None:
    """Record in BLOCKED_NOTIFY_FILE that we've sent a DM for this task.

    Called BEFORE delivering the wake event so that even if delivery fails
    we don't double-notify on the next tick.  The entry is cleaned up by
    clean_blocked_notify_tracker() when the task leaves stuck/blocked-human.
    """
    if DRY_RUN:
...
def check_and_notify_blocked_human_tasks() -> None:
    ...
    for task in blocked_tasks:
        ...
        # Record BEFORE delivering so even a delivery error won't cause
        # double-notification on the next heartbeat tick.
        record_blocked_human_notified(task_id, task_title)

        log(f"  Sending blocked-human notification for "
            f"{task_id[:8]}: {task_title}")

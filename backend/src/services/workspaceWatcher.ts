 ...
 * Tracks: AGENTS.md, HEARTBEAT.md, IDENTITY.md, SOUL.md, TOOLS.md, USER.md, SKILLS.md, memory/, skills/
 */
export class WorkspaceWatcher {
  ...
  private static readonly CORE_FILES: Record<string, string> = {
    ...
    'SKILLS.md': 'Workspace skill index',
  };
  ...
  public getFiles(): WorkspaceFile[] {
    return Array.from(this.fileCache.values()).sort((a, b) => {
      ...
      if (a.category !== b.category) {
        const order: Record<string, number> = { core: 0, skills: 1, memory: 2, other: 3 };
  ...
  public async getFileContent(name: string): Promise<{ content: string; meta: WorkspaceFile } | null> {
    ...
    try {
      ...
      const meta: WorkspaceFile = {
        ...
        description: WorkspaceWatcher.CORE_FILES[name] || (name.startsWith('memory/') ? 'Daily memory log' : name.startsWith('skills/') ? 'Workspace skill' : 'Workspace file'),
      };
  ...
  private async scanAll() {
    ...
    const skillsScanDir = path.join(this.workspacePath, 'skills');
    try {
      const skillDirs = await readdir(skillsScanDir);
      for (const dir of skillDirs) {
        const skillFile = path.join(skillsScanDir, dir, 'SKILL.md');
        try {

...
export class GatewayConnector {
  ...
  /** Tracks cron session keys that were spawned as task agents (via spawn-agent endpoint) */
  private taskSessionKeys: Set<string> = new Set();
  ...
  /**
   * Register a cron session key as a task-spawned agent so anti-prune logic protects it.
   */
  public registerTaskSession(key: string): void {
    this.taskSessionKeys.add(key);
    console.log(`📌 GatewayConnector: Registered task session: ${key}`);
  }

  /**
   * Unregister a task session key (called on manual cleanup or completion).
   */
  public unregisterTaskSession(key: string): void {
    this.taskSessionKeys.delete(key);
    console.log(`🗑️  GatewayConnector: Unregistered task session: ${key}`);
  }

  /**
   * Check whether a session key is a tracked task agent.
   */
  public isTaskSession(key: string): boolean {
    return this.taskSessionKeys.has(key);
  }
  ...
  private async fetchSessions(): Promise<void> {
    try {
      ...
      for (const session of sessions) {
        ...
        const isTaskAgent = this.taskSessionKeys.has(session.key);
        const isTrackedAgent = isSubagent || isTaskAgent;
        ...
        if (isGenerating && isTrackedAgent && existing) {
      ...
      for (const [key, session] of this.sessionStates.entries()) {
        if (!updatedKeys.has(key)) {
          ...
          const isTaskAgentKey = this.taskSessionKeys.has(key);
          if ((isSubagentKey || isTaskAgentKey) && session.tokenUsage.total === 0 && (now - session.lastActivity) < 2 * 60 * 60 * 1000) {
            continue;
          }

          // Move to historical
          // Auto-cleanup task session registration when the session moves to historical
          this.taskSessionKeys.delete(key);

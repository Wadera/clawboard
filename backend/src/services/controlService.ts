...
import { findMainSessionEntry, OpenClawSessionEntry } from './openclawState';
...
export class ControlService {
  ...
  constructor(sessionsPath: string, configPath?: string) {
    ...
    this.configPath = configPath || process.env.OPENCLAW_CONFIG_PATH || process.env.CLAWDBOT_CONFIG_PATH || '/clawdbot/clawdbot.json';
  }
  ...
  async stopMain(): Promise<StopResult> {
    try {
      ...
      const sessions: Record<string, OpenClawSessionEntry> = JSON.parse(data);

      // OpenClaw 2026.6.11 renamed the live main session key to
      // agent:<id>:explicit:main; the legacy agent:main:main entry is frozen.
      // findMainSessionEntry prefers the live key and falls back to legacy.
      ...
      const sessionKey = mainEntry.key;
  ...
  private async abortSession(sessionKey: string): Promise<boolean> {
    const gatewayUrl = process.env.OPENCLAW_GATEWAY_WS_URL || process.env.CLAWDBOT_GATEWAY_WS_URL || 'ws://host.docker.internal:18789';

...
function runtimePathCandidates(targetPath: string): string[] {
  ...
  if (targetPath.startsWith('/home/clawd/clawd/projects/')) {
    const suffix = targetPath.slice('/home/clawd/clawd/projects/'.length);
    add(path.join('/task-projects', suffix));
    add(path.join('/workspace/projects', suffix));
  }

  if (targetPath.startsWith('/home/clawd/clawd/')) {
    add(targetPath.replace('/home/clawd/clawd', '/workspace'));
  }

  if (targetPath.startsWith('/srv/ai-stack/projects/nimspace/')) {
    const suffix = targetPath.slice('/srv/ai-stack/projects/nimspace/'.length);
    add(path.join('/task-projects', suffix));
    add(path.join('/project-sources', suffix));
  }

  if (targetPath.startsWith('/mnt/nfs/NimsProjects/')) {
    const suffix = targetPath.slice('/mnt/nfs/NimsProjects/'.length);
    add(path.join('/project-sources', suffix));

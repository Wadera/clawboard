...
const sessionsPath = process.env.OPENCLAW_SESSIONS_PATH || process.env.CLAWDBOT_SESSIONS_PATH;
const LEGACY_SESSIONS_DIR = sessionsPath
...
const LEGACY_NOTIFICATIONS_FILE = path.join(LEGACY_SESSIONS_DIR, 'task-notifications.json');
...
export class NotificationManager {
  ...
  private async readNotificationData(): Promise<NotificationData> {
    for (const candidate of [NOTIFICATIONS_FILE, LEGACY_NOTIFICATIONS_FILE]) {
      try {
        const content = await fs.readFile(candidate, 'utf8');
        return JSON.parse(content);
      } catch (err: any) {
        if (err.code && err.code !== 'ENOENT') {
          console.warn(`[NotificationManager] Error reading notifications file ${candidate}:`, err.message);
        }
      }
    }

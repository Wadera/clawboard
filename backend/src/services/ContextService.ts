...
export class ContextService {
  async buildContext(projectId: string, options: ContextOptions): Promise<any> {
    ...
    // Build merged localPaths: structured resources + DB-level source_dir/nfs_dir
    const mergedLocalPaths: Record<string, string | null> = {};
    // DB-level fields take priority for source_dir and nfs_dir
    if (project.source_dir) {
      mergedLocalPaths.sourceDir = `/project-sources/${project.source_dir}`;
    }
    if (project.nfs_dir) {
      mergedLocalPaths.nfsRoot = `${process.env.NFS_PROJECTS_ROOT || '/mnt/nfs/projects'}/${project.nfs_dir}`;
    }
    // Then merge structured localPaths (won't overwrite DB-level if set)
    if (structuredResources.localPaths) {
      if (structuredResources.localPaths.nfsRoot && !mergedLocalPaths.nfsRoot) {
        mergedLocalPaths.nfsRoot = structuredResources.localPaths.nfsRoot;
      }
      if (structuredResources.localPaths.ssdBuild) {
        mergedLocalPaths.ssdBuild = structuredResources.localPaths.ssdBuild;
      }
      if (structuredResources.localPaths.dockerCompose) {
        mergedLocalPaths.dockerCompose = structuredResources.localPaths.dockerCompose;
      }
    }

    // Separate doc links for a dedicated documentation section
    const docLinks = otherLinks.filter(l => l.type === 'doc' || l.type === 'file');
    const nonDocLinks = otherLinks.filter(l => l.type !== 'doc' && l.type !== 'file');
    ...
    const payload: any = {
      ...
      resources: {
        ...
        localPaths: Object.keys(mergedLocalPaths).length > 0 ? mergedLocalPaths : {},
        links: nonDocLinks.map(l => ({ type: l.type, title: l.title, url: l.url })),
        documentation: docLinks.length > 0
          ? docLinks.map(l => ({ title: l.title, url: l.url, type: l.type }))
          : [],
      },
      files: {
      ...
      if (task) {
        const taskContext: any = {
          ...
          subtasks: task.subtasks.map(s => ({ 
            text: s.text, 
            status: s.status || (s.completed ? 'completed' : 'empty'),
...
- Use PATCH /tasks/{taskId}/subtasks/{index}/status with {"status": "review", "role": "agent"}
...
- Review 'review' subtasks and approve or reject them
...
- Task can only be 'completed' when ALL subtasks are 'completed'
    ...
    // Priority 10: Project reports (linked documentation/research)
    if (currentTokens < budget) {
      try {
        const reportsResult = await reportManager.list({ project_id: projectId, limit: 10 });
        if (reportsResult.reports.length > 0) {
          const reportLinks = reportsResult.reports.map(r => ({
          ...
          const test = { ...payload, reports: reportLinks };
          if (estimateTokens(test) <= budget) {
            payload.reports = reportLinks;
            currentTokens = estimateTokens(payload);
          }
        }
      } catch {
        // Reports table not available (migration not run) — skip silently
      }
    }

...
router.get('/:id/source-files', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // source_dir can be a repo-relative path like "myproject/repo" — resolve it directly
    // Check if source_dir looks like a path (contains /) vs a plain name
    let baseDir: string;
    if (projectName.includes('/')) {
      // Treat as relative path under SOURCE_BASE_PATH
      const safePath = projectName.split('/').map(s => s.replace(/[^a-zA-Z0-9._-]/g, '')).join('/');
      const resolvedDir = path.join(SOURCE_BASE_PATH, safePath);
      baseDir = fs.existsSync(resolvedDir) && fs.statSync(resolvedDir).isDirectory() ? resolvedDir : path.join(SOURCE_BASE_PATH, safePath);
    } else {
      const safeName = projectName.replace(/[^a-zA-Z0-9._-]/g, '');
      const projectDir = path.join(SOURCE_BASE_PATH, safeName);
      const repoDir = path.join(projectDir, 'repo');
      baseDir = fs.existsSync(repoDir) && fs.statSync(repoDir).isDirectory() ? repoDir : projectDir;
    }

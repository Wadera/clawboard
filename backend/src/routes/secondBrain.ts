...
// The kf-broker may be LAN-bound on your network — set KF_BROKER_URL accordingly.
...
function brokerBase(): string {
  return (process.env.KF_BROKER_URL || 'http://localhost:8940').replace(/\/$/, '');
}
...
// The Qdrant web UI can sit behind a reverse proxy — set QDRANT_UI_URL to it
...
const QDRANT_UI_URL = process.env.QDRANT_UI_URL || 'http://localhost:6333/dashboard';
const COOKIE_DOMAIN = process.env.QDRANT_COOKIE_DOMAIN || '';

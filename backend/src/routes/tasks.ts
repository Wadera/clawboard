...
router.patch('/:id', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    if (updates.status) {
      const role = req.body?.role || (req.headers['x-clawboard-role'] as string) || 'orchestrator';
...
router.post('/:id/spawn-agent', async (req: Request, res: Response): Promise<void> => {
  try {
    ...
    // Register with GatewayConnector so anti-prune logic protects this cron session
    gatewayConnector.registerTaskSession(childSessionKey);
